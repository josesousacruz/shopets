...
const FinalizarVendaModal: React.FC<FinalizarVendaModalProps> = ({
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.id_forma_pagamento || formData.id_forma_pagamento <= 0) {
      newErrors.id_forma_pagamento = 'Selecione uma forma de pagamento';
    }

    if (formData.pontos_fidelidade_utilizados && clienteSelecionado) {
      const pontosDisponiveis = clienteSelecionado.pontos_fidelidade || 0;
      if (formData.pontos_fidelidade_utilizados > pontosDisponiveis) {
        ...
        >
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <select
                value={formData.id_forma_pagamento}
                onChange={(e) => handleInputChange('id_forma_pagamento', parseInt(e.target.value))}
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                  errors.id_forma_pagamento ? 'border-red-300' : 'border-gray-300'
                }`}
              >
                ...
                {formasPagamento.map((forma) => (
                  <option key={forma.id_forma_pagamento} value={forma.id_forma_pagamento}>
                    {forma.nome} ({forma.tipo.replace('_', ' ').toUpperCase()})
                  </option>
                ))}
              </select>

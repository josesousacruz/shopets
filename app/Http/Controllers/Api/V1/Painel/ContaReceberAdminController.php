...
class ContaReceberAdminController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        $q = ContaReceber::query()
            ->with('cliente:id_cliente,nome')
            ->where('ativo', true)
            ->when($request->query('status'), fn ($qq, $v) => $qq->where('status', $v))
            ->when($request->query('id_cliente'), fn ($qq, $v) => $qq->where('id_cliente', $v))
    ...
    public function store(Request $request): JsonResponse
    {
        $data = $request->validate([
            ...
            'tipo_documento' => ['nullable', 'in:duplicata,promissoria,cheque,boleto,outros'],
        ...
        $criadas = DB::transaction(function () use ($data, $idPdv, $request, $parcelas, $intervalo, $valorParcela) {
            ...
            for ($i = 0; $i < $parcelas; $i++) {
                ...
                $conta = ContaReceber::create([
                    ...
                    'id_conta_origem' => $base?->id_conta_receber,
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        ...
        $data = $request->validate([
            ...
            'categoria' => ['sometimes', 'in:venda_prazo,servico,outros'],
            'observacoes' => ['nullable', 'string'],
        ]);
        $conta->update($data);
    ...
    public function baixar(Request $request, int $id): JsonResponse
    {
        ...
        if ($conta->status === 'recebido') {
            throw ValidationException::withMessages(['status' => 'Conta já está recebida.']);
        }
        $data = $request->validate([
        ...
        $conta->update([
            'status' => 'recebido',
            'valor_recebido' => $data['valor_recebido'] ?? $conta->valor_original,
    ...
    {
        ContaReceber::findOrFail($id)->update(['ativo' => false, 'status' => 'cancelado']);
    ...
    {
        $base = ContaReceber::where('ativo', true);
        ...
        return [
            ...
            'recebido_mes' => (float) (clone $base)->where('status', 'recebido')
                ->whereBetween('data_recebimento', [now()->startOfMonth()->toDateString(), now()->endOfMonth()->toDateString()])
                ->sum('valor_recebido'),
        ];
    }
